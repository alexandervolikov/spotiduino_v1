...
uint8_t pass[4] = {0, 0, 0, 0}; //пароль станции - первые три байта, дополнительный пароль на запись nfc-чипов
...
void rfid() {
  ...
  //Если пароль нули или 255, то не используется авторизации для записи, иначе используется. Если специальный пароль nfc 0, то не используется также
